...
# You know that a unit fraction is a fraction with a numerator of 1 and a denominator of a positive integer. You also know that the sum of any number 
# of unit fractions is always a rational number.
...
def getEFU(num, den, loD):
    if num == 0:
        return loD
    newDemon = math.ceil(den/num)
    loD.append(newDemon)
    loD = getEFU(num*newDemon - den, newDemon*den, loD)
    return loD
