...
# Example:
# For n = 2 and m = 2, the output should be legoWall(n, m) = 3. For n = 3 and m = 2, the output should be legoWall(n, m) = 7. For n = 2 and m = 3, the output should be legoWall(n, m) = 9.
# Input/Output:
# 1.The input consists of two integers: n and m, representing the desired height and width of the wall, respectively. 2.The output is a long integer representing the number of ways to build the wall, modulo 1000000007.

# Exercise-1 Input: 2 2
# Here First row - n value Second row - m value
# Output: 3

# Exercise-2 Input: 4 4
# Output: 3375

def legoBlocks(n,m):
    MOD = (10**9 + 7)
    row_combo = [1,1,2,4]
    while len(row_combo) <= m:
        row_combo.append(sum(row_combo[-4:])%MOD)
    
    total = [pow(c,n,MOD) for c in row_combo]
    unstable = [0,0]
